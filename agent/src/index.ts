    ...
    for (const plugin of character.plugins) {
        if (plugin.clients) {
            for (const client of plugin.clients) {
                console.log("Client ka structure", client);

                // Access the actual client with the 'start' function inside the 'clients' array
                const twitterClient = client.clients?.[0]; // Accessing the first client (assuming one is there)

                if (
                    twitterClient &&
                    twitterClient.start &&
                    typeof twitterClient.start === "function"
                ) {
                    const startedClient = await twitterClient.start(runtime);
                    elizaLogger.debug(`Initializing client: ${client.name}`);
                    clients.push(startedClient);
                } else {
                    elizaLogger.error(
                        `client.start function nahi mil raha client: ${client.name}`
                    );
                }
            }
        }
